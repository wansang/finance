...
- **구조**:
  ...
  3. 🚨 [지금진입가능관심주] 섹션을 만들어서, 현재 매수 신호가 포착된 종목들을 강조해줘. 각 종목마다 다음 두 가지 전문가 의견을 반드시 포함해줘:
     - **[주식 전문가 의견]**: 차트·기술 분석 관점에서 지금 이 가격에 진입해도 되는지, 추가로 확인해야 할 사항(거래량 지속 여부, 다음 저항선 위치 등)을 1~2문장으로 설명해줘.
     - **[ETF·포트폴리오 전문가 의견]**: 해당 종목이 속한 섹터 흐름과 시장 전체 모멘텀을 고려했을 때 지금 진입이 적절한지, 분할 매수 또는 일괄 매수 중 어느 쪽이 나은지를 1~2문장으로 설명해줘.
     데이터가 '없음'이면 이 섹션은 "현재 진입 가능한 관심종목이 없습니다."라고 짧게 작성해줘.
...
"""
    ...
    def calculate_entry_price(self, df, code, is_etf=False):
        ...
        # 목표가: BB 중단(현재가 위일 때만) 또는 진입가 +수익목표%
        profit_target_pct = self.config.get('PROFIT_TARGET_PCT', 0.08)
        if bbm and bbm > close:
            target = round(bbm, 0)
            target_basis = "BB중단"
        else:
            target = round(entry * (1 + profit_target_pct), 0)
            target_basis = f"+{profit_target_pct*100:.0f}%"

        # R/R 최소 1.5배 보장 (목표 수익 < 손절 폭 × 1.5이면 목표 상향)
        risk = entry - stop_loss
        reward = target - entry
        if risk > 0 and reward < risk * 1.5:
            target = round(entry + risk * 1.5, 0)
            target_basis = "R/R 1.5배 기준"
