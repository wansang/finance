...
- **구조**:
  ...
  3. 🚨 [지금진입가능관심주] 섹션을 만들어서, 현재 매수 신호가 포착된 종목들을 강조해줘. 어떤 신호가 발생했는지, 진입가/손절/목표가를 명확히 언급하고, 지금 당장 행동해야 할 이유를 설명해줘. 데이터에 '📊 [주식전문가]' 또는 '🌐 [ETF전문가]' 의견이 포함되어 있으면 해당 의견을 그대로 포함하되, 전문가 의견을 새로 추가 작성하지 말 것. 데이터가 '없음'이면 이 섹션은 "현재 진입 가능한 관심종목이 없습니다."라고 짧게 작성해줘.
