...
if [[ -z "${PROJECT_ID:-}" || -z "${LOCATION:-}" || -z "${GITHUB_PAT:-}" ]]; then
  cat <<EOF
Usage: PROJECT_ID=<project> LOCATION=<location> GITHUB_PAT=<token> bash google-scheduler/create_agent_search_job.sh
...
JOB_NAME="agent-search-dispatch"
URI="https://api.github.com/repos/wansang/finance/actions/workflows/agent_search.yml/dispatches"
SCHEDULE="0 23 * * 5"  # UTC: 금요일 23:00 => KST 토요일 08:00
